...
const { FrameButton } = require('../src/components/FrameButton/FrameButton.tsx');
const { ImageButton } = require('../src/components/ImageButton/ImageButton.tsx');
const { SkyrimHint } = require('../src/components/SkyrimHint/SkyrimHint.tsx');
const { SkyrimSlider } = require('../src/components/SkyrimSlider/SkyrimSlider.tsx');
const { SkyrimFrame } = require('../src/components/SkyrimFrame/SkyrimFrame.tsx');
const { SkyrimButton } = require('../src/components/SkyrimButton/SkyrimButton.tsx');
const { SkyrimInput } = require('../src/components/SkyrimInput/SkyrimInput.tsx');
...
run('collectServerTags returns unique sorted tags', () => {
  const servers = [
    { id: '1', name: 'A', ip: '1.1.1.1', players: 1, maxPlayers: 2, ping: 10, tags: ['pve', 'eu'] },
    { id: '2', name: 'B', ip: '1.1.1.2', players: 1, maxPlayers: 2, ping: 20, tags: ['pvp', 'eu', ''] },
...
run('getVisibleServers filters full servers and search query', () => {
  const servers = [
    ...
    { name: 'Gamma', ip: '192.168.1.5', players: 4, maxPlayers: 10, ping: null }
  ];

  const noFull = getVisibleServers(servers, '', 'players', false);
  assert.deepEqual(noFull.map((s) => s.name), ['Alpha', 'Gamma']);

  const searchByName = getVisibleServers(servers, 'ga', 'players', true);
  assert.deepEqual(searchByName.map((s) => s.name), ['Gamma']);

  const searchByIp = getVisibleServers(servers, '10.0.0.2', 'players', true);
  assert.deepEqual(searchByIp.map((s) => s.name), ['Beta']);
});
...
run('getVisibleServers sorting respects players, ping and name', () => {
  ...
  assert.deepEqual(byPlayers.map((s) => s.name), ['Alpha', 'Bravo', 'Charlie']);

  const byPing = getVisibleServers(servers, '', 'ping', true);
  assert.deepEqual(byPing.map((s) => s.name), ['Bravo', 'Alpha', 'Charlie']);

  const byName = getVisibleServers(servers, '', 'name', true);
  assert.deepEqual(byName.map((s) => s.name), ['Alpha', 'Bravo', 'Charlie']);
});

run('getVisibleServers supports favorites and tag filters', () => {
  const servers = [
    { id: '1', name: 'Alpha', ip: '10.0.0.1', players: 2, maxPlayers: 20, ping: 90, tags: ['pve'] },
    { id: '2', name: 'Beta', ip: '10.0.0.2', players: 3, maxPlayers: 20, ping: 50, tags: ['pvp'] },
    { id: '3', name: 'Gamma', ip: '10.0.0.3', players: 4, maxPlayers: 20, ping: 70, tags: ['pve', 'hardcore'] }
  ];
  ...
  assert.deepEqual(onlyFavorites.map((s) => s.id), ['2', '3']);

  const byTag = getVisibleServers(servers, '', 'players', true, {
    requiredTag: 'pve',
  });
  assert.deepEqual(byTag.map((s) => s.id), ['3', '1']);
});
...
run('filterAdminPlayers matches by userId, actorName and ip', () => {
  ...
  assert.deepEqual(filterAdminPlayers(players, '12').map((p) => p.userId), [12]);
...
run('SkyrimButton renders text and disabled opacity', () => {
  const html = render(React.createElement(SkyrimButton, {
    name: 'loginSubmit',
    text: 'Login',
    width: 300,
    height: 50,
...
run('SkyrimInput renders label, placeholder and initial value', () => {
  const html = render(React.createElement(SkyrimInput, {
    name: 'email',
    type: 'email',
    labelText: 'E-Mail',
...
run('FrameButton renders default variant with active state and text', () => {
  const html = render(React.createElement(FrameButton, {
    name: 'playBtn',
    text: 'Play',
    variant: 'DEFAULT',
    width: 384,
...
run('ImageButton renders image and disabled button style', () => {
  const html = render(React.createElement(ImageButton, {
...
run('SkyrimHint renders open left-aligned active state', () => {
  const html = render(React.createElement(SkyrimHint, {
    isOpened: true,
...
run('SkyrimSlider renders title and slider classes', () => {
  const html = render(React.createElement(SkyrimSlider, {
    text: 'Volume',
    sliderValue: 35,
    min: 0,
    max: 100,
...
run('SkyrimFrame renders outer frame with expected size', () => {
  const html = render(React.createElement(SkyrimFrame, {
    width: 512,
    height: 704,
    header: true
  }));
