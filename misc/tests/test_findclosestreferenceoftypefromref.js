...
const main = async () => {
  ...
  const barrelObject = {
    type: 'form',
    desc: mp.getDescFromId(barrelInWhiterun),
  };

  const barrelBaseForm = {
    type: 'espm',
    desc: mp.get(barrelInWhiterun, 'baseDesc'),
  };

  let searchRes;

  // expect no actor to be found around the barrel
  searchRes = mp.callPapyrusFunction(
    'global',
    'Game',
    'FindClosestReferenceOfTypeFromRef',
    null,
    [akFormToPlace, barrelObject, 100],
  );
  assert.deepEqual(searchRes, null);

  // placing
  const placedActor = mp.callPapyrusFunction(
    'method',
    'ObjectReference',
    'PlaceAtMe',
    barrelObject,
    [akFormToPlace, 1, true, false],
  );
  ...
  searchRes = mp.callPapyrusFunction(
    ...
    'FindClosestReferenceOfTypeFromRef',
    null,
    [barrelBaseForm, placedActor, 100],
  );
