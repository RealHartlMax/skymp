...
export class SpApiInteractor {
  ...
  static getControllerInstance(): CombinedController {
    ...
    SpApiInteractor.controller = {
      ...
      lookupListener<T extends ClientListener>(
        constructor: ClientListenerConstructor<T>,
      ): T {
        const listener =
          SpApiInteractor.listenersForLookupByName.get(constructor);
        if (listener === undefined) {
          throw new Error(`listener not found for name '${constructor.name}'`);
        }
        if (!(listener instanceof constructor)) {
          throw new Error(
            `listener class mismatch for name '${constructor.name}'`,
          );
        }
        return listener;
      },
  ...
  private static registerListenerForLookup(
