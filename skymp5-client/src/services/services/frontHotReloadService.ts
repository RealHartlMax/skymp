...
export class FrontHotReloadService extends ClientListener {
  constructor(private sp: Sp, private controller: CombinedController) {
    ...
    const enable =
      !!this.sp.settings['skymp5-client']['enable-front-hotreload'];
    ...
    const authGameData = this.sp.storage[authGameDataStorageKey] as
      | AuthGameData
      | undefined;
    ...
    if (storageHasValidAuthGameData) {
      logTrace(
        this,
        `Recovered AuthGameData from storage, starting FrontHotReloadService`,
      );
      this.connectToFrontHotReload();
    } else {
      logTrace(
        this,
        `Unable to recover AuthGameData from storage, waiting for auth`,
      );
