...
export class NetworkingService extends ClientListener {
  constructor(private sp: Sp, private controller: CombinedController) {
    ...
    this.controller.emitter.on('sendRawMessage', (e) =>
      this.onSendRawMessage(e),
    );
  ...
  private onSendMessageWithRefrId(e: SendMessageWithRefrIdEvent<AnyMessage>) {
    ...
    const idxInModel = refrId
      ? remoteServer
          .getWorldModel()
          .forms.findIndex((f) => f && f.refrId === refrId)
      ...
      switch (packetType) {
        ...
        case 'message':
          ...
          if (rawContent === null) {
            ...
            new Uint8Array(rawContent as unknown as ArrayBuffer)[0] === 0x7b
          ) {
            // assume json
            msgAny = JSON.parse(
              this.sp.decodeUtf8(rawContent as unknown as ArrayBuffer),
            );
          } else {
            // assume raw
