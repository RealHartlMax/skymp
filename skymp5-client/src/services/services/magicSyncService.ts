...
import {
  SpellCastMessage,
  SpellCastMsgData,
} from '../messages/spellCastMessage';
...
export class MagicSyncService extends ClientListener {
  ...
  private onUpdate() {
    ...
    this.controller.once('update', () => {
      ...
      const animVariables = this.getAnimationVariablesFromActorConverted(
        ac.getFormID(),
      );
  ...
  private onSpellCast(event: SpellCastEvent) {
    ...
    const msg: SpellCastMsgData = this.getSpellCastEventData(
      event,
      isInterruptCast,
    );
  ...
  private onSendAnimationEventLeave(ctx: {
    ...
    if (
      !this.lastSpellCastEventMsg ||
      !this.isInteraptSpellCastAnim(ctx.animEventName)
    ) {
      return;
    }
    ...
    this.controller.once('update', () => {
      if (
        !this.lastSpellCastEventMsg ||
        this.lastSpellCastEventMsg.interruptCast
      ) {
        return;
      }
  ...
  private getSpellCastEventData(
    e: SpellCastEvent,
    isInterruptCast: boolean,
  ): SpellCastMsgData {
    const spellCastData: SpellCastMsgData = {
      ...
      actorAnimationVariables: this.getAnimationVariablesFromActorConverted(
        e.caster.getFormID(),
      ),
    };
  ...
  private isInteraptSpellCastAnim(animEventName: string): boolean {
    ...
    return (
      eventName === 'mlh_equipped_event' || eventName === 'mrh_equipped_event'
    );
  }
  ...
  private isAnyMagicStuffEquiped(): boolean {
    ...
    if (
      ac.getEquippedSpell(SpellType.Left) ||
      ac.getEquippedSpell(SpellType.Right)
    ) {
      return true;
    }

    if (
      ac.getEquippedSpell(SpellType.Voise) ||
      ac.getEquippedSpell(SpellType.Instant)
    ) {
      return true;
    }
