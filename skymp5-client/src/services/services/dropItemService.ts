...
export class DropItemService extends ClientListener {
  ...
  private onContainerChanged(e: ContainerChangedEvent) {
    const sweetCantDropService = this.controller.lookupListener(
      SweetTaffySweetCantDropService,
    );
    ...
    if (
      ...
      const worldCleanerService =
        this.controller.lookupListener(WorldCleanerService);

      set.forEach((refrId) => {
        const ref = this.sp.ObjectReference.from(
          this.sp.Game.getFormEx(refrId),
        );
        if (ref !== null && ref.isDeleted() === false) {
          ...
          if (worldCleanerService.getWcProtection(refrId) === 0) {
            ...
            logTrace(
              this,
              'Found and deleted reference ' + refrId.toString(16),
            );
          } else {
            logTrace(
              this,
              'Found reference ' + refrId.toString(16) + " but it's protected",
            );
          }
        }
      });
