...
export class SweetCameraEnforcementService extends ClientListener {
  constructor(private sp: Sp, private controller: CombinedController) {
    ...
    this.settings = this.sp.settings['skymp5-client']['animDebug'] as
      | AnimDebugSettings
      | undefined;
  ...
  private onCustomPacketMessage2(e: ConnectionMessage<CustomPacketMessage>) {
    ...
    let isPlayExitAnimAfterwardsEnabled =
      content['isPlayExitAnimAfterwardsEnabled'];
    let parentAnimEventName = content['parentAnimEventName'];
    let enablePlayerControlsDelayMs = content['enablePlayerControlsDelayMs'];
    let preferInterruptAnimAsExitAnimTimeMs =
      content['preferInterruptAnimAsExitAnimTimeMs'];
  ...
  private onSendAnimationEventLeave(ctx: {
    animEventName: string;
    animationSucceeded: boolean;
  }) {
    const animLowerCase = ctx.animEventName.toLowerCase();
    if (
      animLowerCase.startsWith('idle') &&
      !animLowerCase.startsWith('idleforcedefaultstate')
    ) {
      this.controller.once('update', () => {
  ...
  private exitAnim(options: ExitAnimOptions) {
    ...
    if (options.playExitAnim) {
      ...
      this.sp.Debug.sendAnimationEvent(
      ...
      this.sp.Utility.wait(enablePlayerControlsDelaySeconds).then(() => {
        this.sp.Game.enablePlayerControls(
          true,
          false,
          true,
          false,
  ...
  private startAntiExploitPolling(mode: 'no_death' | 'death' = 'death') {
    ...
    let f = (i: number): void => {
      ...
      if (cameraState === 0) {
        ...
        this.exitAnim({
          playExitAnim: true,
          enablePlayerControlsDelayMs: undefined,
        });
        if (mode === 'death') {
  ...
  private onButtonEvent(e: ButtonEvent) {
    ...
    if (
      ...
      if (this.needsExitingAnim) {
        ...
        if (this.currentAnim?.options) {
          this.exitAnim({
            playExitAnim: true,
            enablePlayerControlsDelayMs:
              this.currentAnim.options.enablePlayerControlsDelayMs,
          });
        } else {
          this.exitAnim({
            playExitAnim: true,
      ...
      if (this.needsExitingAnim) {
        ...
        if (
          !intervalMs ||
          Date.now() - this.lastNotificationMoment >= intervalMs
        ) {
    ...
    if (animEvent.toLowerCase() === 'idleforcedefaultstate') {
      ...
      const doInvoke = () => {
        ...
        if (
          ...
          this.sp.Utility.wait(
            options.preferInterruptAnimAsExitAnimTimeMs / 1000,
          ).then(() => {
            state.prefer = false;
            logTrace(
              this,
              `Prefer interrupt anim as exit anim state changed to false`,
            );
          });
