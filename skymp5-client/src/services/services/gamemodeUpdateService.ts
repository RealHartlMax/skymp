...
} from '../messages/updateGameModeDataMessage';
...
export class GamemodeUpdateService extends ClientListener {
  ...
  private onUpdateGamemodeDataMessage(
    event: ConnectionMessage<UpdateGamemodeDataMessage>,
  ) {
    if (this.sp.settings['skymp5-client']['disable-gamemode-updates']) {
      ...
      logTrace(
        this,
        `Gamemode updates are disabled by settings - processing first update only`,
      );
      this.sp.storage['GamemodeUpdateService_sawUpdateFunctions'] = true;
    }
  ...
  private onCreateActorMessage(event: ConnectionMessage<CreateActorMessage>) {
    ...
    this.controller.once('tick', () => {
      ...
      if (!myFormModel) {
        logError(
          this,
          `Unable to find formModel with index`,
          worldModel.playerCharacterFormIdx,
        );
        return;
      }
  ...
  private onTick() {
    const keys = this.sp.storage[
      'updateNeighborFunctions_keys'
    ] as Array<string>;
    ...
    this.updateNeighborFunctions = this.sp.storage[
      'updateNeighborFunctions'
    ] as Record<string, any>;
  }

  private onUpdate() {
    const playerRef = this.sp.ObjectReference.from(
    ...
    for (const propName of keys) {
      ...
      this.updateOwnerCtx.value = (
        this.updateOwnerCtx._model as Record<string, unknown>
      )[propName];
  ...
  private updateGamemodeUpdateFunctions(
    ...
    functionSources.forEach(
      (pair) => (functionSourcesRecord[pair.name] = pair.content),
    );
    ...
    this.sp.storage[`${storageVar}_keys`] = Object.keys(
      this.sp.storage[storageVar] as any,
    );
  }
