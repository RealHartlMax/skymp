...
export class GamemodeEventSourceService extends ClientListener {
  constructor(private sp: Sp, private controller: CombinedController) {
    ...
    if (Array.isArray(this.sp.storage['eventSourceContexts'])) {
      this.sp.storage['eventSourceContexts'] = this.sp.storage[
        'eventSourceContexts'
      ].filter((ctx: Record<string, unknown>) => !ctx._expired);
  ...
  private onUpdateGamemodeDataMessage(
    ...
    if (!Array.isArray(this.sp.storage['eventSourceContexts'])) {
      ...
      this.sp.storage['eventSourceContexts'].forEach(
    ...
    event.message.eventSources.forEach(
      (pair) => (eventSourcesRecord[pair.name] = pair.content),
    );

    let eventNames = Object.keys(eventSourcesRecord);

    let blockedEventSources =
      this.sp.settings['skymp5-client']['blockedEventSources'];

    if (Array.isArray(blockedEventSources)) {
      blockedEventSources.forEach((blockedEventSource: unknown) => {
        eventNames = eventNames.filter(
          (eventName) => eventName !== blockedEventSource,
        );
        logTrace(
          this,
          `'eventSources`,
    ...
    eventNames.forEach((eventName) => {
      ...
      try {
        ...
        (this.sp.storage['eventSourceContexts'] as Record<string, any>).push(
          ctx,
        );
        this.setupEventSource(ctx);
      } catch (e) {
