...
export class SettingsService extends ClientListener {
  ...
  public getSelectedServerMasterKey(): string | null {
    const selected = this.sp.storage[
      selectedServerMasterKeyStorageKey
    ] as unknown;
  ...
  public getMasterUrl() {
    return this.normalizeUrl(
      (this.sp.settings['skymp5-client']['master'] as string) ||
  ...
  public getTargetPeer(callback?: TargetPeerCallback): {
    targetPeerCached: TargetPeer | null;
  } {
  ...
  private getTargetPeerImpl(callback: TargetPeerCallback) {
    ...
    const states = {
      start: () => {
        try {
          if (
            this.sp.settings['skymp5-client']['server-info-ignore'] as boolean
          ) {
            logTrace(
              this,
              'Skipping serverinfo request due to server-info-ignore in config',
            );
          ...
          this.controller
            ...
            .setTimeout(
              () =>
                states.reject(
                  new Error('getTargetPeer: serverinfo request timed out'),
                ),
              serverInfoRequestTimeoutMs,
            );

          let headers: HttpHeaders = {};
          let session = this.controller
            .lookupListener(AuthService)
            .readAuthDataFromDisk()?.session;
          if (session) {
      ...
      resolve: (targetPeer: TargetPeer) => {
        ...
        enrichedTargetPeer.publicKeys = {
          ...defaultPeer.publicKeys,
          ...targetPeer.publicKeys,
        };
      ...
      reject: (err: unknown) => {
        ...
        logTrace(
          this,
          `Server info request failed, falling back to`,
          defaultPeer,
          `; error:`,
          err,
        );
  ...
  public async getServerMods(): Promise<Mod[]> {
    ...
    for (let attempt = 0; attempt < 5; ++attempt) {
      try {
        ...
        const res = await masterApiClient.get(
