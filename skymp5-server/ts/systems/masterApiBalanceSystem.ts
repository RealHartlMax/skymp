...
export class MasterApiBalanceSystem implements System {
  ...
  async initAsync(ctx: SystemContext): Promise<void> {
    const listenerFn = (userId: number, session: string) => {
      console.log(
        `MasterApiBalanceSystem.userAssignSession - Assigning session for userId ${userId}`,
      );
    ...
    (ctx.svr as any).getUserMasterApiBalance = async (
      userId: number,
    ): Promise<number> => {
      if (this.offlineMode) {
        console.log(
          'MasterApiBalanceSystem.getUserMasterApiBalance - Always zero balance in offline mode',
        );
        return 0;
      }
      ...
      if (!session) {
        console.error(
          `MasterApiBalanceSystem.getUserMasterApiBalance - Invalid session value for userId ${userId} (session = ${session})`,
        );
        throw new Error(
          `MasterApiBalanceSystem.getUserMasterApiBalance - Invalid session value for userId ${userId} (session = ${session})`,
        );
    ...
    (ctx.svr as any).makeUserMasterApiPurchase = async (
      userId: number,
      balanceToSpend: number,
    ): Promise<{ balanceSpent: number; success: boolean }> => {
      if (this.offlineMode) {
        console.log(
          'MasterApiBalanceSystem.makeUserMasterApiPurchase - Purchase impossible in offline mode',
        );
      ...
      if (!session) {
        console.error(
          `MasterApiBalanceSystem.makeUserMasterApiPurchase - Invalid session value for userId ${userId} (session = ${session})`,
        );
        throw new Error(
          `MasterApiBalanceSystem.makeUserMasterApiPurchase - Invalid session value for userId ${userId} (session = ${session})`,
        );
  ...
  private async getUserBalanceImpl(session: string): Promise<number> {
    try {
      ...
      if (
        ...
        typeof response.data.user.balance !== 'number'
      ) {
        throw new Error(
          `getUserBalanceImpl: bad master-api response ${JSON.stringify(
            response.data,
          )}`,
        );
      }
  ...
  private async makeUserMasterApiPurchaseImpl(
    session: string,
    balanceToSpend: number,
  ): Promise<{ balanceSpent: number; success: boolean }> {
    try {
      ...
      if (
        !response.data ||
        typeof response.data.balanceSpent !== 'number' ||
        typeof response.data.success !== 'boolean'
      ) {
        throw new Error(
          `makeUserMasterApiPurchaseImpl: bad master-api response ${JSON.stringify(
            response.data,
          )}`,
        );
      }
  ...
  connect(userId: number, ctx: SystemContext) {
    console.log(
      `MasterApiBalanceSystem.connect - Cleaning session for userId ${userId}`,
    );
  ...
  disconnect(userId: number, ctx: SystemContext) {
    console.log(
      `MasterApiBalanceSystem.disconnect - Cleaning session for userId ${userId}`,
    );
    this.sessionByUserId[userId] = undefined;
  }
