...
| *Admin Dashboard Features* | 🔄 In Progress | Tabs (players/console/logs/resources/cfg), player search, ban/unban (permanent + timed), kick with reason, mute/unmute with countdown badges, send-message-to-player flow, console command sender, filtered activity log, and ban/mute file persistence are implemented; resources inventory is now focused on Skyrim-relevant files (`.esm`/`.pex`) and server-settings CFG editor includes structured Access/Discord form validation before save; richer moderation workflows (warn, cross-panel quick search) still pending |
...
| *Respawn & Revival Configuration* | 🔄 In Progress | Server-settings CFG editor now includes spawn point management (`startSpawn`/`startPoints`) and starter inventory configuration; custom respawn control system (`canRespawn` property) is available in backend and enables healer/doctor revival mechanics; dedicated revival system UI panel (player downed status, manual revive actions) remains planned |
